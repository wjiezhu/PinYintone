...
## 2. 分组发码
- **A 组（静态色块）**：发 `1` 开头的 6 位码，如 `100001`、`100002`…
- **B 组（动态 F0）**：发 `2` 开头，如 `200001`、`200002`…
- 每人发不同尾号便于区分个体。被试注册时**必须填码**（已强制）。

## 3. 被试操作（发 PARTICIPANT_GUIDE.md 给他们）
装 TestFlight → 装 App → 允许麦克风 → 选「我是学生」→ 填码 → 三关卡练习。
...
- 分组：班级码前缀（1→A / 2→B / 其余&游客→B），客户端本地判定。
